...
try:
    # 读取原文件
    with open('/Volumes/共享盘/openclaw/main/projects/trainer-ai-tool/client/src/views/Dashboard.vue', 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    # 新的模板内容
    new_template = '''    <div v-show="activeTab === 'grading'" class="page-view">
      ...
      <div class="toolbar">
        <a-input v-model="pendingGradingSearch" placeholder="搜索考生姓名..." style="width: 200px" @input="filterPendingGrading" allow-clear>
        ...
        <a-button @click="resetPendingGradingFilter" v-if="pendingGradingSearch || pendingGradingPaperFilter">重置</a-button>
      </div>
      
      <a-card class="content-card">
        <div v-if="filteredPendingGradingList.length === 0" style="text-align: center; padding: 60px 0; color: var(--text-secondary)">
        ...
        <a-table v-else :data="filteredPendingGradingList" :columns="pendingGradingColumns" row-key="id" :pagination="false" :bordered="false">
...
'''
    ...
    # 替换第 734-778 行（索引 733-777）
    new_lines = lines[:733] + [new_template + '\n'] + lines[778:]
    
    # 写回文件
    with open('/Volumes/共享盘/openclaw/main/projects/trainer-ai-tool/client/src/views/Dashboard.vue', 'w', encoding='utf-8') as f:
