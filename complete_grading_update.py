...
# 要替换的旧内容
old_table_content = '''        <a-table v-else :data="filteredPendingGradingList" :columns="pendingGradingColumns" row-key="id" :pagination="false" :bordered="false">
...
# 新的表格内容
new_table_content = '''        <a-table v-else :data="filteredPendingGradingList" :columns="pendingGradingColumns" row-key="id" :pagination="false" :bordered="false">
...
# 替换内容
if old_table_content in content:
    new_content = content.replace(old_table_content, new_table_content)
