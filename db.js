  ...
  users: {
    ...
    delete: (id) => {
      ...
      if (index !== -1) {
        ...
        const userPapers = db.papers.filter(p => p.user_id === deletedUserId);
        const paperKeyIds = userPapers.map(p => p.key_id).filter(k => k);
        db.papers = db.papers.filter(p => p.user_id !== deletedUserId);
        db.questions = db.questions.filter(q => q.user_id !== deletedUserId);
        db.categories = db.categories.filter(c => c.user_id !== deletedUserId);
        db.examRecords = db.examRecords.filter(e => e.user_id !== deletedUserId);
        db.paperStudents = db.paperStudents.filter(ps => ps.user_id !== deletedUserId);
        db.scoreRecords = db.scoreRecords.filter(sr => sr.user_id !== deletedUserId);
