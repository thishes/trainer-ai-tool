...
const factory = (env) => {
  ...
  const encodeText = isFetchSupported && (typeof TextEncoder === "function" ? ((encoder) => (str) => encoder.encode(str))(new TextEncoder()) : (str) => __async(void 0, null, function* () {
...
axios.default = axios;
const axios$1 = axios;
const baseURL = {}.VITE_API_BASE_URL || "/api";
...
let retryCounts = /* @__PURE__ */ new Map();
...
api.interceptors.response.use((response) => {
  ...
  if (originalConfig._retryCount === void 0) {
    originalConfig._retryCount = 0;
  }
  const shouldRetry = isNetworkError || RETRY_CONFIG.retryNetworkErrors.includes(errorCode) || status && RETRY_CONFIG.retryStatusCodes.includes(status);
  ...
  if (shouldRetry && originalConfig._retryCount < RETRY_CONFIG.maxRetries) {
    ...
    yield sleep(delay);
    try {
      const response = yield api(originalConfig);
      clearRetryCount(originalConfig);
      return response;
    } catch (retryError) {
...
const getCaptcha = () => api.get("/auth/captcha");
const getUserInfo = () => api.get("/auth/me");
const logout = () => api.post("/auth/logout");
const getQuestions = (params) => api.get("/questions", {
  params
});
const createQuestion = (data) => api.post("/questions", data);
const updateQuestion = (id, data) => api.put(`/questions/${id}`, data);
...
const getPapers = (params) => api.get("/papers", {
  params
});
const getPaper = (id) => api.get(`/papers/${id}`);
const getPaperPublic = (id, params) => api.get(`/papers/public/${id}`, {
  params
});
const createPaper = (data) => api.post("/papers", data);
const deletePaper = (id) => api.delete(`/papers/${id}`);
const publishPaper = (id) => api.post(`/papers/${id}/publish`);
const unpublishPaper = (id) => api.post(`/papers/${id}/unpublish`);
const createRandomPaper = (data) => api.post("/papers/random", data);
const getPaperExamUrl = (id) => api.get(`/papers/${id}/exam-url`);
const getPaperQuestions = (paperId) => api.get(`/papers/${paperId}/questions`);
const addQuestionsToPaper = (paperId, questionIds) => api.post(`/papers/${paperId}/questions`, {
  question_ids: questionIds
});
const removeQuestionFromPaper = (paperId, questionId) => api.delete(`/papers/${paperId}/questions/${questionId}`);
const startExamApi = (data) => api.post("/exam/start", data);
const getExamQuestions = (examId) => api.get(`/exam/${examId}/questions`);
const saveProgress = (data) => api.post("/exam/save-progress", data);
const submitExam = (data) => api.post("/exam/submit", data);
const getExamResult = (examId) => api.get(`/exam/${examId}/result`);
const getExamRecords = (paperId, params) => api.get(`/exam/records/${paperId}`, {
  params
});
const getExamStats = (paperId) => api.get(`/exam/stats/${paperId}`);
const getPendingGrading = (paperId, params) => api.get(`/exam/pending-grading/${paperId}`, {
  params
});
const gradeEssay = (data) => api.post("/exam/grade-essay", data);
const getAnnouncements = (params) => api.get("/announcements", {
  params
});
const createAnnouncement = (data) => api.post("/announcements", data);
const updateAnnouncement = (id, data) => api.put(`/announcements/${id}`, data);
...
const getPaperStudents = (paperId) => api.get(`/students/paper/${paperId}`);
const addPaperStudents = (paperId, studentIds) => api.post(`/students/paper/${paperId}`, {
  student_ids: studentIds
});
const removePaperStudent = (paperId, studentId) => api.delete(`/students/paper/${paperId}/${studentId}`);
const exportPaperStudents = (paperId) => {
  const baseURL2 = {}.VITE_API_BASE_URL || "";
  return fetch(`${baseURL2}/api/students/paper/${paperId}/export`, {
    headers: {
      "X-Requested-With": "XMLHttpRequest"
    },
    credentials: "include"
...
const checkUpgrade = () => api.get("/upgrade/check");
const doUpgrade = (version) => api.post("/upgrade/upgrade", {
  version
});
...
const lockUser = (id, status) => api.patch(`/users/${id}/status`, {
  status
});
const deleteUser = (id) => api.delete(`/users/${id}`);
const changePassword = (data) => api.post("/users/change-password", data);
const getPromotions = (params) => api.get("/promotions", {
  params
});
const getPromotion = (id) => api.get(`/promotions/${id}`);
const createPromotion = (data) => api.post("/promotions", data);
const updatePromotion = (id, data) => api.put(`/promotions/${id}`, data);
const deletePromotion = (id) => api.delete(`/promotions/${id}`);
const lockPromotion = (id) => api.post(`/promotions/${id}/lock`);
const unlockPromotion = (id) => api.post(`/promotions/${id}/unlock`);
const getPromotionPreview = (id) => api.get(`/promotions/${id}/preview`);
const getPromotionPublic = (id) => api.get(`/promotions/${id}/public`);
const getPromotionSignups = (id, params) => api.get(`/promotions/${id}/signups`, {
  params
});
const createPromotionSignup = (id, data) => api.post(`/promotions/${id}/signups`, data);
const signupPromotion = (id, data) => api.post(`/promotions/${id}/signups`, data);
const createPromotionSignupManual = (id, data) => api.post(`/promotions/${id}/signups/manual`, data);
const updatePromotionSignup = (id, signupId, data) => api.put(`/promotions/${id}/signups/${signupId}`, data);
const deletePromotionSignup = (id, signupId) => api.delete(`/promotions/${id}/signups/${signupId}`);
const updatePromotionSignupStatus = (id, signupId, status) => api.patch(`/promotions/${id}/signups/${signupId}/status`, {
  status
});
const exportPromotionSignups = (id) => api.get(`/promotions/${id}/signups/export`, {
  responseType: "blob"
});
export {
  doUpgrade as $,
  deleteUser as A,
  getAnnouncements as B,
  updateAnnouncement as C,
  ...
  unpublishPaper as N,
  deletePaper as O,
  createRandomPaper as P,
  createPaper as Q,
  addPaperStudents as R,
  createStudent as S,
  removePaperStudent as T,
  exportPaperStudents as U,
  importStudents as V,
  getPaperStudents as W,
  getPaperExamUrl as X,
  getExamRecords as Y,
  getExamStats as Z,
  checkUpgrade as _,
  getUserInfo as a,
  changePassword as a0,
  getPaper as a1,
  getPaperQuestions as a2,
  addQuestionsToPaper as a3,
  removeQuestionFromPaper as a4,
  getPaperPublic as a5,
  startExamApi as a6,
  getExamQuestions as a7,
  saveProgress as a8,
  submitExam as a9,
  getExamResult as aa,
  getPromotion as ab,
  signupPromotion as ac,
  getPromotionPublic as ad,
  createPromotionSignup as ae,
  logout as b,
  api as c,
  getPromotionPreview as d,
  getPromotionSignups as e,
  exportPromotionSignups as f,
  getCaptcha as g,
  createPromotionSignupManual as h,
  updatePromotionSignupStatus as i,
  deletePromotionSignup as j,
  getPromotions as k,
  login as l,
  updatePromotion as m,
  createPromotion as n,
  deletePromotion as o,
  unlockPromotion as p,
  lockPromotion as q,
  register as r,
  getPapers as s,
  getPendingGrading as t,
  updatePromotionSignup as u,
  gradeEssay as v,
  getUsers as w,
  updateUser as x,
  createUser as y,
